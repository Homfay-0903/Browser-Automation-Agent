...
// The browser session id a finished run drove, read from its final output so
// a panel can potentially use it. Only the output carries it.
function sessionIdForRun(run: WorkflowRun): string | undefined {
  return run.output?.browserSessionId
}
...
export interface ConsoleRun {
  ...
  isLive: boolean
  steps: RunStep[]
  // The browser session id (for potential replay integration)
    ...
    () =>
      [...runs]
        ...
        .map((run) => ({
          ...
          browserSessionId: sessionIdForRun(run),
        })),
    [runs]
  )
}
