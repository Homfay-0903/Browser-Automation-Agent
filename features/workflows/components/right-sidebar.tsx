...
// A single editor field for a node property. Renders a checkbox when `boolean` is
// set, a multi-line textarea when `multiline` is set, otherwise a single-line input.
function Field({
  field,
  value,
  onChange,
...
export function RightSidebar({ workflowId }: { workflowId: string }) {
  ...
  // Auto-switch to the Editor tab when the user selects a node on the canvas.
  // Track the previously-selected node id in a ref (not state) so the effect can
  // detect transitions without triggering an extra render.
