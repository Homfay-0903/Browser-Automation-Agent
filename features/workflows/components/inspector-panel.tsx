...
export function InspectorPanel({ selection }: { selection: ConsoleSelection }) {
  ...
  // Note: Session replay requires Browserbase or a similar service with built-in
  // recording. Browserless doesn't provide native session recordings.
  if (selection.kind === "replay") {
    if (!run?.browserSessionId) {
      return <Note>This recording is no longer available.</Note>
    }
    // Session replay is available with Browserbase. For Browserless,
    // we show a placeholder since native recordings aren't supported.
    return (
      <div className="flex size-full flex-col items-center justify-center gap-3 p-4 text-center">
        ...
        <div className="text-xs text-muted-foreground">
          Session replay requires Browserbase or a similar recording-enabled browser service.
          <br />
          Current session ID: {run.browserSessionId}
        </div>
      </div>
    )
  }
