...
export const runWorkflowTask = task({
  ...
  run: async ({ workflowId, orgId }: { workflowId: string; orgId: string }) => {
    ...
    const getStagehand = async () => {
      ...
      // If init is already in progress, await the same promise to avoid creating
      // a second instance (defensive — the sequential for-loop makes this unlikely).
      if (stagehandPromise) return stagehandPromise

      stagehandPromise = (async () => {
        // Determine LLM provider and model configuration
        const llmProvider = process.env.LLM_PROVIDER || "glm"
        let modelConfig: { modelName: string; apiKey: string; baseURL?: string }

        if (llmProvider === "deepseek") {
          // Stagehand V3 has native DeepSeek support via @ai-sdk/deepseek
          modelConfig = {
            modelName: "deepseek/deepseek-chat",
            apiKey: process.env.DEEPSEEK_API_KEY!,
          }
        } else {
          // Default to GLM (智谱) via OpenAI-compatible API
          modelConfig = {
            modelName: "openai/glm-4-flash",
    ...
    for (let i = 0; i < order.length; i++) {
      ...
      try {
        ...
        publishSteps()
        await metadata.flush()
        // Shutdown errors (e.g. CDP WebSocket compression mismatch) are
        // harmless — the run already succeeded; just log and move on.
        await stagehand?.close().catch((e) =>
          logger.log(`Browser cleanup: ${(e as Error).message}`),
        )
        throw error
      }
